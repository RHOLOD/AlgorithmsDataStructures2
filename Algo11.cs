    ...
    public class SimpleGraph<T>
    {
        ...
        public List<Vertex<T>> BreadthFirstSearch(int VFrom, int VTo)
        {
            ...
            {             
                ...
                {
                    int number = 0;
                    var item = queue.Dequeue().Value;
                    if (!item.Equals(vertex[VFrom].Value))
                    {
                        for (int i = 0; i < max_vertex; i++)
                        {
                            if (vertex[i].Value.Equals(item))
                            {
                                number = i;
                                break;
                            }
                        }
                        stack.Push(vertex[number]);
                        if (FindingPath(number)) return true;
                        else stack.Pop();
                    }
